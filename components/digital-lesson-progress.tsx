...
export function DigitalLessonProgress({ orderId, releaseId, lessonId, completed, labels }: { orderId: string; releaseId: string; lessonId: string; completed: boolean; labels: { complete: string; completed: string; error: string } }) {
  ...
  async function toggle() {
    ...
    try { const res = await fetch("/api/digital/progress", { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ orderId, releaseId, lessonId, completed: !completed }) }); if (!res.ok) setError(true); else router.refresh(); }
    catch { setError(true); } finally { setBusy(false); }
  }
  return <div><button type="button" aria-pressed={completed} disabled={busy} onClick={toggle} className="mt-4 min-h-11 rounded-xl border border-line px-4 py-2 text-sm font-semibold disabled:opacity-60">{busy ? "…" : completed ? `✓ ${labels.completed}` : labels.complete}</button>{error && <p role="alert" className="mt-2 text-sm text-danger-text">{labels.error}</p>}</div>;
}
