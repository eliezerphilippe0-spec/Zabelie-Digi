...
export async function DigitalSellerMetrics({ sellerId, lang }: { sellerId: string; lang: Lang }) {
  ...
  return <section className="mt-8 rounded-2xl border border-line p-5"><h2 className="titre-section">{t(lang, "studio.stats")}</h2>{error || !values ? <p role="alert" className="mt-3 text-sm text-mist">{t(lang, "studio.statsUnavailable")}</p> : <dl className="mt-5 grid gap-5 sm:grid-cols-3">{(Object.keys(values) as (keyof typeof values)[]).map(key => <div key={key}><dt className="text-sm text-mist">{t(lang, `studio.${key}`)}</dt><dd className="numeric mt-1 text-2xl font-bold">{values[key]}</dd></div>)}</dl>}<p className="mt-5 text-sm text-mist">{t(lang, "studio.statsHint")}</p></section>;
}
