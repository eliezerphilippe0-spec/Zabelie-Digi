...
export function ProductOffers({ offers, copy, afterPurchase = false }: { offers: PublicOffer[]; copy: OfferCopy; afterPurchase?: boolean }) {
  ...
  const card = (offer: PublicOffer) => <article key={offer.id ?? offer.target_product_id} className="rounded-xl border border-line bg-surface p-4">
    <p className="text-xs font-semibold text-accent">{offer.origin === "purchases" ? copy.alsoBought : copy[offer.offer_kind]}</p>
  ...
  return <section aria-label={afterPurchase ? copy.after : copy.buyerTitle} className="mt-6 space-y-3 text-left">
    ...
    {visible.filter(o => o.offer_kind === "downsell").map(offer => <details key={offer.id ?? offer.target_product_id} className="rounded-xl border border-line p-3">
