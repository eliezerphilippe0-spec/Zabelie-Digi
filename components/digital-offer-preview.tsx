import { formatDigitalSize, type DigitalManifest } from "@/lib/digital-studio";
...
export function DigitalOfferPreview({ manifest: m, lang }: { manifest: DigitalManifest; lang: Lang }) {
  return <section className="mt-6 space-y-6 rounded-2xl border border-line bg-surface p-5" aria-label={t(lang, "studio.preview")}>
    ...
    {m.files.length > 0 && <div><h2 className="text-lg font-bold">{t(lang, "studio.files")}</h2><ul className="mt-3 space-y-2">{m.files.map(f => <li key={f.id} className="flex flex-wrap justify-between gap-2 border-b border-line py-2"><span className="break-all">{f.file_name}</span><span className="numeric text-sm text-mist">{formatDigitalSize(f.size_bytes, lang)}</span></li>)}</ul></div>}
