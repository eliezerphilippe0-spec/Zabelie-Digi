...
APPLIQUÉE en production le 2026-08-21**, PR-T1), **`0088`** fonctions d'émission
et de scan (PR-T3/T4), **`0089`** plafonds si le payant s'ouvre.

⚠️ **DÉCALÉES D'UN CRAN LE 2026-08-21, ET C'EST LA DEUXIÈME FOIS.** Ce document
annonçait `0087` pour l'émission ; le numéro a été pris entre-temps par
`0087_rail_gratis.sql` (rail gratuit), qui est un fichier réel du dépôt. Une
spec ne réserve pas un numéro : **le disque tranche, pas le plan.**

La première occurrence est encore visible sur la ligne PR-T3 du tableau §7 —
elle annonçait `0086`, pris par PR-T1. Deux fois le même piège sur le même
document en deux jours : **le numéro à écrire dans une spec est celui qu'on
mesure au moment de rédiger la migration, jamais celui qu'on avait prévu.**

```bash
ls supabase/migrations | sed 's/_.*//' | sort -n | tail -1   # le dernier pris
```
