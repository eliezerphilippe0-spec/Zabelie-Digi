...
| **D-8** | **Le retrait à la demande (`zabelie_request_payout`, `0034`) constitue-t-il une rétention de fonds ou une opération de cash-out au sens de la Circulaire 121 ?** | Réglementaire + comptable. **Ce n'est pas un écart de documentation : le code et la règle se contredisent.** `0034_payout_requests.sql` est **appliqué en production** ; C121 est la raison même pour laquelle le versement devait être automatique à maturité J+7, jamais à la demande. **Ce qui débloque** : réponse **écrite** du Cabinet Volmar. **Si compatible** → le chantier 0 (`docs/19`) reprend, et la règle du versement automatique J+7 devient un **défaut**, non une interdiction. **Si incompatible** → `0034` et `zabelie_request_payout` sont **désactivés par feature flag, PAS supprimés**, et `docs/19` est réécrit. **Suspendu** : chantier 0. ⛔ Ne rien supprimer avant cette réponse. | **Ouverte — en attente du Cabinet Volmar** |
