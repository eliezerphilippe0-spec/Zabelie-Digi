...
| V-19 | **Connexion et inscription par fournisseurs tiers — Google, Microsoft, Facebook, Apple** | **Décision porteur (2026-09-03, « je veux ajouter des possibilités de connexion/inscription, google, microsoft etc. »).** Ce sont des services externes au sens de `CLAUDE.md` (« aucun service externe non listé sans validation ») : la validation est cette demande. Mécanisme : Supabase Auth (OAuth PKCE), `lib/auth-providers.ts`, boutons dans `components/connexion-form.tsx`, retour par `/auth/callback` existant. **Règle de conception : rien ne s'affiche sans configuration** — la liste vient de `NEXT_PUBLIC_AUTH_PROVIDERS`, posée APRÈS l'activation du fournisseur dans Supabase, sinon un bouton mène à une page d'erreur brute hors interface (`signInWithOAuth` navigue, il ne lève pas). Un compte né par fournisseur passe par le même déclencheur de profil (`0045`), qui lit désormais `full_name`/`name` (`0095`) sous le même filtre anti-usurpation. **Ce que ça ne tranche pas** : quels fournisseurs activer. Google d'abord — chaque Android en a un ; Facebook est le réseau dominant en Haïti ; Microsoft et Apple servent surtout la diaspora. Les identifiants de chaque fournisseur (client ID/secret) vivent dans le tableau de bord Supabase, jamais dans Vercel ni dans le dépôt (`docs/11` §2). Runbook : `OPS_TODO`. |
| V-20 | **Accueil premium — trois arbitrages par délégation** | **Décision porteur du 2026-09-04 (« go » sur le plan de Phase 0, puis « sans préférence » aux trois questions posées).** Le plan (`docs/home-premium/PLAN.md` §7) relevait quatre contradictions entre le brief et le dépôt ; le porteur a délégué, l'agent a pris l'option recommandée à chaque fois : (1) **le français reste la langue par défaut** — V-18 tenue, le brief demandait le kreyòl ; (2) **Manrope + Inter remplacent Playfair Display** — la demande du 2026-08-11 est retirée par son auteur via le brief §3.2 ; (3) **palette du brief, toile crème par défaut, sombre conservé sur cookie** — l'indigo `#17123a` abandonné le 2026-07-25 revient comme encre et `theme-color`, sur une toile claire où l'argument de la barre d'adresse violette tombe. Quatre valeurs du brief sont AJUSTÉES par la porte de contraste, mesures dans `app/zabelie-theme.css` : brand `#e0580f` en clair (le `#f26a21` du brief rend 2,89:1), accent texte `#b8500c`, surface `#fffdf9` (pas de blanc pur), pas de troisième orange `#fdb868`. **Ce que ça ne tranche pas** : la langue dans l'URL (`docs/47` §3), et le hex exact de la marque si le porteur veut `#f26a21` en clair au prix de A9. |
| V-18 | **Le français reste la langue par défaut** | **Décision porteur (2026-09-02, « Oui fr »).** `getLang()` retombait sur `fr` depuis l'origine sans que personne ne l'ait choisi ; l'audit pré-déploiement (constat #4) et `docs/47` §3 l'avaient posé comme arbitrage ouvert. Tranché : sans cookie — premier visiteur, lien partagé, crawler — le site sert le **français**. Le kreyòl reste à un clic et dans le cookie. **Ce que ça ne tranche pas** : la langue dans l'URL (`/ht/`, `/fr/`), seule voie pour que le kreyòl soit indexé — `docs/47` §3 reste ouvert, et son coût monte avec le catalogue (83 points de contact au 2026-08-28). Garde : `lib/i18n-server.ts` porte la décision en commentaire. |
...
| **D-7** | **Un vendeur vérifié peut-il vendre du crédit Digicel/Natcom ?** | Commercial + réglementaire. Distincte de V-17 : « Zabelie ne vend plus de minutes » ne dit **pas** « des vendeurs peuvent en vendre ». Un crédit télécom est un **bien**, pas de la monnaie électronique — la question est donc recevable, contrairement à la revente de solde MonCash/NatCash qui reste **interdite** (`docs/07-TOPUP.md` §3, `docs/17`). Ouverte par effet de bord de V-17 ; **volontairement non implémentée** et non tranchée. | **Volet COMMERCIAL tranché le 2026-09-05** — porteur : *« rajoute la section, en cas d'interdit je vais l'enlever »*, après deux mises en garde (avis juridique, ligne « rechaj » / « vann balans ») et réaffirmation. `0098` ouvre les trois rayons et donne un sous-rayon à tout produit ; retour arrière = trois `update … set active = false`, au journal d'`OPS_TODO`. **Volet RÉGLEMENTAIRE ouvert** — le porteur le porte sciemment. Historique : *« Zabelie est un intermédiaire […] le vendeur crée sa boutique et Zabelie a une commission sur chaque vente »* — donc **le volet COMMERCIAL penche vers oui**. Le volet **RÉGLEMENTAIRE reste entier** : il demande un avis sur la revente de crédit télécom par un tiers, et la ligne écrite entre « rechaj » et « vann balans ». Deux sous-rayons sont semés **dormants** par `0097` en prévision ; ils n'ouvrent rien. |
