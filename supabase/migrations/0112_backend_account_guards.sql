select zabelie_migration_garde('0112_backend_account_guards.sql');

-- Audit backend 2026-09-22: serialize new orders with seller moderation.
create function public.zabelie_order_seller_guard()
...
begin
  ...
  -- FOR SHARE conflicts with the non-key UPDATE that suspends/closes a profile.
...
-- A suspended/closed storefront must not remain accessible by its old ID or slug.
create or replace function public.zabelie_boutik_public(p_id uuid default null, p_slug text default null)
returns jsonb language plpgsql security definer set search_path=public
as $$
declare v_p profiles;
begin
  if (p_id is null) = (p_slug is null) then return null; end if;
  select * into v_p from profiles
    where ((p_id is not null and id=p_id) or (p_slug is not null and boutik_slug=p_slug))
      and suspended_at is null;
  if not found then return null; end if;
  if v_p.role <> 'creator' and not exists(select 1 from products where seller_id=v_p.id) then
    return null;
  end if;
  return jsonb_build_object(
    'id',v_p.id,'display_name',v_p.display_name,'bio',v_p.bio,'avatar_url',v_p.avatar_url,
    'zone_id',v_p.zone_id,'pwen_repe',v_p.pwen_repe,'boutik_slug',v_p.boutik_slug);
end;
$$;
revoke all on function public.zabelie_boutik_public(uuid,text) from public;
grant execute on function public.zabelie_boutik_public(uuid,text) to anon,authenticated,service_role;

-- Signed delayed failure: lock the same payment as confirm_payment.
-- A late failure must never cancel a confirmed payment or another session.
create function public.zabelie_stripe_payment_failed(p_order_id uuid, p_session_id text, p_event_id text)
returns text language plpgsql security definer set search_path=public
as $$
declare v_payment payments;
begin
  select * into v_payment from payments where order_id=p_order_id for update;
  if not found or v_payment.rail <> 'stripe' then
    raise exception 'Stripe payment unavailable' using errcode='ZB112';
  end if;
  if nullif(p_session_id,'') is null or nullif(p_event_id,'') is null or
     (v_payment.raw->>'stripe_session_id' is not null and v_payment.raw->>'stripe_session_id' <> p_session_id) then
    raise exception 'Stripe session mismatch' using errcode='ZB112';
  end if;
  if v_payment.status <> 'pending' then return v_payment.status::text; end if;
  update payments set status='failed',
    raw=coalesce(raw,'{}'::jsonb)||jsonb_build_object('stripe_session_id',p_session_id,'stripe_failure_event_id',p_event_id)
...
$$;
revoke all on function public.zabelie_stripe_payment_failed(uuid,text,text) from public,anon,authenticated;
grant execute on function public.zabelie_stripe_payment_failed(uuid,text,text) to service_role;
