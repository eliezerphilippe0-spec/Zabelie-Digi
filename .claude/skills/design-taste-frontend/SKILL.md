...
---
<!-- ═══════════════════════════════════════════════════════════════════════════
     ZABELIE — PRÉCÉDENCE. LIRE AVANT LE RESTE.

     Ce fichier est `skills/taste-skill/SKILL.md` de https://github.com/Leonxlnx/taste-skill
     (v2 experimental, MIT, commit ccbc156 du 2026-08-24), copié VERBATIM sous
     ce bloc. Empreinte SHA-256 du fichier amont, c'est-à-dire de ce fichier
     RECONSTITUÉ SANS ce bloc (frontmatter + corps, octet pour octet) :
       aa194351b246b8b4799099d4ed7b033d29eab6e6e3d58d8d2172978be7b3ec89
     `tests/skill-taste.test.ts` la recalcule à chaque passage : un
     `npx skills add` qui remplacerait ce fichier en place effacerait ce
     bloc, une retouche du corps amont changerait l'empreinte, et dans les
     deux cas la suite rougit. Pour mettre à niveau : recopier l'amont, poser
     ce bloc, mettre à jour l'empreinte ET le commit cité.

     RÈGLE : quand ce skill et la doctrine du dépôt (CLAUDE.md, docs/25 §4,
     app/zabelie-theme.css, tests/*) se contredisent, LE DÉPÔT GAGNE, et la
     contradiction se SIGNALE au porteur au lieu de se résoudre seule. Le
     skill est un instrument de goût ; il n'est pas une autorité.

     1. PÉRIMÈTRE (§13 du skill, appliqué à Zabelie). Il vaut pour les
        surfaces d'atterrissage : `/` (accueil), `/boutik/[slug]`,
        `/createur/[id]`, `/aide`, `/vendre` (la page d'appel, pas le
        formulaire). Il NE vaut PAS pour le tunnel de paiement, `/panier`,
        `/rechaj`, `/tableau-de-bord`, `/mes-achats`, `/mes-ventes`,
        `/admin/*`, ni pour aucun formulaire. Là, dire « hors périmètre » et
        ne rien appliquer.

     2. PALETTE ET CONTRASTE. La palette est celle de `app/zabelie-theme.css`
        (décision du 2026-07-25, `app/layout.tsx`). Le mode « Redesign —
        Preserve » (§11) est le seul mode admis : extraire les tokens
        existants, jamais en inventer. La porte mécanique de contraste est
        `npm run check:contrast` (bloquante en CI), pas l'estimation du skill.

     3. TIRET CADRATIN (§9.G). Le français et le kreyòl emploient le tiret
        cadratin comme ponctuation légitime ; `lib/i18n.ts` en porte 271 en
        quatre langues et AUCUN ne se réécrit sous cette règle. Le §9.G
        s'applique au texte NEUF que l'agent rédige comme décoration
        (accroches, étiquettes, attribution), pas au corpus traduit.

     4. TYPOGRAPHIE (§4.1). Le dépôt utilise Inter + Playfair Display
        (`app/layout.tsx`), deux choix que le skill déconseille par défaut.
        Les changer est une décision de marque : à PROPOSER au porteur avec
        l'argument du skill, jamais à appliquer dans un tour de refonte.

     5. ICÔNES ET MOTION (§3.A, §3.C, §9.E). Aucune bibliothèque d'icônes ni
        de motion n'est installée, et c'est voulu : Android d'entrée de gamme,
        3G, coupures. Les SVG inline minimaux et les keyframes CSS sous
        `@media (prefers-reduced-motion: no-preference)` (`app/globals.css`)
        SONT la convention. Ajouter Phosphor, Motion ou GSAP est une
        dépendance nouvelle : zone d'arrêt (docs/25 §4), proposer, ne pas
        installer.

     6. IMAGES (§4.8). Zabelie est un « trust-first commerce » au sens du
        §0.A du skill : les contraintes de confiance PRIMENT sur l'esthétique.
        Aucune photo de remplissage (picsum, stock) sur une place de marché où
        l'image est la marchandise ; l'outil d'image validé est Higgsfield
        (CLAUDE.md). Sans visuel réel : état vide honnête, pas de décor.

     7. TEXTE. Toute chaîne visible passe par `lib/i18n.ts` en quatre langues
        (fr, ht, en, es) — `tests/i18n-chaines-en-dur.test.ts` et
        `tests/i18n-cles-mortes.test.ts` l'imposent. Le skill ne produit
        jamais de copie en dur dans un composant.

     8. DIALS. Lecture par défaut pour Zabelie : « trust-first / regulated »
        (§1.A) → DESIGN_VARIANCE 3-4, MOTION_INTENSITY 2-3, VISUAL_DENSITY 4-5.
        Pas la ligne « landing page (default) » à 8/6/4.

     Installé le 2026-09-02 à la demande du porteur. Les douze autres skills du
     même dépôt (redesign, minimalist, brutalist, image-to-code, brandkit…) ne
     sont pas installés ; `redesign-skill` serait le second candidat utile.
     ═══════════════════════════════════════════════════════════════════════ -->
