...
export async function PUT(req: Request) {
  ...
  const { productId, offers } = body as Record<string, unknown>;
  const selection = parseOfferSelection(offers);
  if (typeof productId !== "string" || !OFFER_UUID.test(productId) || !selection) return NextResponse.json({ error: copy.invalid }, { status: 400 });
  const { data, error } = await createAdminClient().rpc("zabelie_save_product_offers", { p_user_id: user.id, p_product_id: productId, p_targets: selection });
