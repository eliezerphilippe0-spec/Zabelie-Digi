...
import { UUID_RE, releaseAllowed, type DigitalRelease } from "@/lib/digital-studio";
...
export default async function DigitalLibraryPage({ params, searchParams }: { params: Promise<{ orderId: string }>; searchParams: Promise<{ version?: string }> }) {
  ...
  return shell(<>
    ...
    {release.payload.lessons.length > 0 && <section className="mt-8"><h2 className="text-xl font-bold">{t(lang, "studio.lessons")}</h2>{progressError ? <p role="alert" className="mt-3 text-danger-text">{t(lang, "studio.error")}</p> : <div className="mt-3"><label htmlFor="course-progress" className="text-sm">{t(lang, "studio.progress", { done: String(done), total: String(release.payload.lessons.length) })}</label><progress id="course-progress" value={done} max={release.payload.lessons.length} className="mt-2 block w-full accent-brand"/></div>}
