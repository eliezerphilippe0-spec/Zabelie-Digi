...
| **⛔ Zone d'arrêt — « au-delà de 50/jour, paiement supplémentaire » (évoqué par le porteur le 2026-08-15)** : rien n'est câblé, et rien ne doit l'être sans arbitrage. Les questions ouvertes : (1) **le jeu en vaut-il la chandelle ?** — une suggestion Gemini flash coûte une fraction de centime ; 50/j × 100 vendeurs à plein régime ≈ quelques dollars/jour au pire, et le trafic réel actuel est à des ordres de grandeur en dessous : la machinerie d'encaissement coûterait plus cher que ce qu'elle récupère ; (2) **par quel rail encaisser ?** — débiter le solde vendeur touche le grand livre (écriture compensatoire obligatoire, invariant 0033) et la rétention BRH (`docs/17`) ; un paiement MonCash à l'acte pour quelques gourdes est disproportionné ; (3) si un jour c'est fait, le quota devient un **paramètre commercial** et doit migrer en table de config (règle dure n°3), plus une ligne CGU. Recommandation en l'état : garder 50/j gratuit ferme, réexaminer si la mesure d'usage réel le justifie. | 2026-08-15 | Rien aujourd'hui — au-delà de 50, le vendeur voit « limite du jour atteinte, revenez demain » et peut toujours écrire lui-même. ⚠️ **Poser d'abord un plafond de dépense dans la console du fournisseur** (OpenAI : Settings → Limits ; Google AI Studio : quotas) — c'est, avec le débit borné côté code (5/min et 60/jour par vendeur), la double ceinture du coût. Clé côté serveur (pas `NEXT_PUBLIC`), redéploiement simple suffit. La clé ne transite jamais par le chat. | 2026-08-14 | Le bouton « Ede m ekri deskripsyon an » des deux formulaires vendeur — construit **dormant** (décision porteur du jour : fournisseurs OpenAI/Google, pas Claude) : sans clé, aucun bouton, la route répond 503, zéro dépense. La suggestion remplit le champ, le vendeur relit et corrige — rien n'est publié automatiquement. |
