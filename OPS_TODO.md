...
| 📮 **Question 6 (comptes de test bac à sable) est arrivée APRÈS l'envoi — elle demande une relance.** Le courriel Digicel est parti le 2026-08-21 avec les questions 1→5 ; la question 6 a été rédigée le même jour, mais **après**. Elle est dans `docs/42` §1 pour la trace, et le fil de relance du 2026-09-01 est l'occasion de la poser — **ne pas renvoyer le courriel entier**, un fournisseur qui reçoit deux fois la même demande répond une fois de moins. | 2026-08-21 | Le parcours de `docs/05`. Sans compte payeur de test, la seule voie de preuve du chemin de l'argent est la **première commande réelle** (`docs/22`), montant minimal et remboursement immédiat. |
