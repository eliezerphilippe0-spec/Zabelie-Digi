...
Provider Higgsfield (#277), routes + journal (#278, `0118`) fusionnés ; facturation (`0119`) en PR. **Studio éteint** (`ZABELIE_STUDIO_ENABLED` absent → routes 404). Décision porteur « oui déduction » : 3 images gratuites/jour, puis **10 HTG** avec consentement au prix, dette prélevée au prochain retrait par le rail du surplus IA (aucun solde prépayé). Reste au porteur, **dans cet ordre** : (1) reconnecter Supabase à la session (le connecteur ne voit plus aucun projet depuis le 2026-09-24 ~14:15Z) ; (2) vérifier que `0071`/`0072` sont appliquées — sinon `0119` échoue, c'est voulu ; (3) appliquer `0118` puis `0119` ; (4) ligne CGU du service payant (docs/34 §3) ; (5) clés Higgsfield **neuves** dans Vercel (Sensitive, jamais `NEXT_PUBLIC_`) ; (6) `ZABELIE_STUDIO_ENABLED=true`. ⚠️ Poser le drapeau avant (3) ferait chercher aux routes des tables absentes. ⚠️ Préexistant, non corrigé ici : un vendeur qui a une dette IA ou Studio ne peut pas supprimer son compte (`zabelie_ai_surplus.seller_id` sans cascade, 0071). Voir `docs/62` §9-§10.
