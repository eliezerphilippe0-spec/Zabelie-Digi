...
| **Appliquer `0077` (taxonomie niveau 3 — V-3 de `docs/35`)** — rédigée, non appliquée. 468 sous-catégories, TOUTES inactives (arbitrage porteur 2026-08-15 : « tout seeder, activer par vagues ») ; l'ouverture d'un rayon devient un `UPDATE ... set active = true`, sans migration. KR/EN best-effort agent, relecture native en attente (registre). Post-conditions : plancher 450, zéro activation. Signal : « applique 0077 ». | 2026-08-15 | Rien à l'écran tant qu'un rayon n'est pas activé — le seed dort, exactement comme docs/16 le voulait. |
