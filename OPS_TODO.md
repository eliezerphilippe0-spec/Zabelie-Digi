...
| **Rangée « Produits gratuits » de l'accueil — MORTE** (constat 2026-08-08, trouvé en tranchant F). `app/page.tsx:110` filtre `priceHTG === 0`, mais `app/api/products/route.ts:65` rejette `price < 1` à la publication : **aucun produit gratuit ne peut exister**. La garde `HomeRow` masque la rangée à vide, donc l'anomalie est invisible. Deux sorties : autoriser le prix 0 (et l'arbitrage F de `docs/29` redevient ouvert), ou retirer la section et ses clés i18n. | 2026-08-08 | Rien — mais c'est du code qui prétend offrir une fonctionnalité que le système interdit. |
