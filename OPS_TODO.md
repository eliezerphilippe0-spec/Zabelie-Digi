...
|---|---|---|
| **Ventes flash — bornes de config** (durée max 24 h · rabais 10–70 % · 3 offres simultanées/vendeur, propositions `docs/37` §B) | 2026-08-15 | La migration `zabelie_flash_sales` — mécanique sans arbitrage d'argent (prix vendeur, commission inchangée), seule la config attend. |
| **Affiliation — QUI PAIE la commission** (A plateforme / B vendeur opt-in recommandée / C agents JForce, `docs/37` §A) | 2026-08-15 | Toute la construction affiliation. Même famille que D-6 : un prélèvement sur le net vendeur ne se décide pas à sa place. Le seuil KYC affilié rejoint le dossier Volmar (`docs/36`). |
